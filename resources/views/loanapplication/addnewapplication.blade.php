			...
			<div class="row">
				...
				<div class="col-md-3 mb-2">
					...
					<select name="loanType" id="loantype" class="form-control text-center" required>
						<option value="" disabled {{ !isset($loan->loanType) ? 'selected' : '' }}>Select Loan Type</option>
						<option value="new" {{ isset($loan->loanType) && $loan->loanType == 'microloan' ? 'selected' : '' }}>New</option>
						<option value="renew" {{ isset($loan->loanType) && $loan->loanType == 'microloan' ? 'selected' : '' }}>Renew</option>
				...
				<div class="col-md-4 mb-3">
					...
					<select name="clientId" id="client" class="form-control" required>
						<option value="" disabled {{ !isset($loan->clientId) ? 'selected' : '' }}>Select a client</option>
						@foreach ($clients as $client)
						<option value="{{$client->id }}" {{ isset($loan->clientId) && $loan->clientId == $client->id ? 'selected' : '' }}>
